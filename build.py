...
hbb_name = 'escradesk' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: escradesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: ESCRA <info@escra.de>
Homepage: https://escra.de
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/escradesk')
    system2('mkdir -p tmpdeb/etc/escradesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/escradesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/escradesk || true')
    #system2(
    #    f'cp -r {flutter_build_dir}/* tmpdeb/usr/lib/escradesk/')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/escradesk/')
    system2(
        'cp ../res/escradesk.service tmpdeb/usr/share/escradesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/escradesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/escradesk.svg')
    system2(
        'cp ../res/escradesk.desktop tmpdeb/usr/share/applications/escradesk.desktop')
    system2(
        'cp ../res/escradesk-link.desktop tmpdeb/usr/share/applications/escradesk-link.desktop')
    #system2(
    #    'cp ../res/com.escradesk.EscraDesk.policy tmpdeb/usr/share/polkit-1/actions/')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/escradesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/escradesk/')
    system2(
        'cp ../res/pam.d/escradesk.debian tmpdeb/etc/pam.d/escradesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/escradesk/files/polkit && chmod a+x tmpdeb/usr/share/escradesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb escradesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('escradesk.deb', '../escradesk-%s.deb' % version)
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libescradesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/libescradesk.dll"):
    ...
    system2(
        f'python ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/escradesk.exe')
    os.chdir('../..')
    if os.path.exists('./EscraDesk-portable.exe'):
        os.replace('./target/release/escradesk-portable-packer.exe',
                   './EscraDesk-portable.exe')
    else:
        shutil.copy('./target/release/escradesk-portable-packer.exe', './EscraDesk-portable.exe')
        print(f'output location: {os.path.abspath(os.curdir)}/EscraDesk-portable.exe')
    shutil.copy('./EscraDesk-portable.exe', f'./EscraDesk-{version}-install.exe')
    print(f'output location: {os.path.abspath(os.curdir)}/EscraDesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/escradesk.exe')
        system2('mv target/release/escradesk.exe target/release/escradesk-portable.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\escradesk.exe')
        else:
            print('Not signed')
        system2('copy target/release/escradesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/escradesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/escradesk-{version}-win7-install.exe ../..')
