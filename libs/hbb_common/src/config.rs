...
lazy_static::lazy_static! {
    ...
    // @bahadir Online is keeping servers and latences of the servers... We also check whether the server is up by this variable. 
    static ref ONLINE: Mutex<HashMap<String, i64>> = Default::default();
    pub static ref PROD_RENDEZVOUS_SERVER: RwLock<String> = RwLock::new("".to_owned());
    pub static ref EXE_RENDEZVOUS_SERVER: RwLock<String> = Default::default();
    pub static ref APP_NAME: RwLock<String> = RwLock::new("EscraDesk".to_owned());
...
pub const LINK_DOCS_HOME: &str = "https://www.escra.de";
pub const LINK_DOCS_X11_REQUIRED: &str = "https://www.escra.de";
pub const LINK_HEADLESS_LINUX_SUPPORT: &str =
    "https://www.escra.de";
lazy_static::lazy_static! {
    pub static ref HELPER_URL: HashMap<&'static str, &'static str> = HashMap::from([
        ("escradesk docs home", LINK_DOCS_HOME),
        ("escradesk docs x11-required", LINK_DOCS_X11_REQUIRED),
        ("escradesk x11 headless", LINK_HEADLESS_LINUX_SUPPORT),
        ]);
}
...
pub const RENDEZVOUS_SERVERS: &[&str] = &["desk.escra.net"];
pub const RS_PUB_KEY: &str = "aaa";

pub const EXTERNAL_ID_SERVER : &str = "desk.escra.net:443";
pub const EXTERNAL_RELAY_SERVER: &str = "desk.escra.net:80";
pub const EXTERNAL_API_SERVER : &str = "";
pub const EXTERNAL_KEY : &str = "aaa";
...
// @bahadir, giving a path, it gives us the related directory for something.. 
...
impl Config2 {
    // @bahadir, afaiu this loads the config file with suffix 2 and stores it in object.
    fn load() -> Config2 {
    ...
    // @bahadir, gets the info from object and encyrpts it and then stores it in toml file with name extension '2'
    fn store(&self) {
...
// @bahadir, it loads information from the given file, in the corresponding type
pub fn load_path<T: serde::Serialize + serde::de::DeserializeOwned + Default + std::fmt::Debug>(
    file: PathBuf,
) -> T {
    let cfg = match confy::load_path(&file) {
        Ok(config) => { log::info!("{}", format!("path to log toml {:?}", file.to_string_lossy())); config},
        Err(err) => {
...
// @bahadir, given the configuration in a type, it will be saved in the path given.
#[inline]
...
impl Config {
    ...
    // @bahadir, ipc refers to the interprocess communication, namely programs sharing data while running concurrently on computer.
    ...
    // @bahadir, first checks exec name, then toml file, then executable variables, 
    // then checks Config2 file, then the object itself.
    ...
    // @bahadir, first checks exec name, then toml file, then executable variables, then checks Config2 file. 
    ...
    // @bahadir, given host and latency, it will check the other hosts and updates the rendezvous server with shortest latency.
    ...
    // @bahadir , '.toml' will be added for the path... 
...
impl PeerConfig {
    ...
    // @bahadir , given an identifier, it makes sure that the path with identifier can be used
    // also it adds .toml extension at the end and returns that path. 
