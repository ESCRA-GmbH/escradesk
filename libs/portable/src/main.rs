...
const APP_PREFIX: &str = "escradesk";
const APPNAME_RUNTIME_ENV_KEY: &str = "ESCRADESK_APPNAME";
#[cfg(windows)]
...
fn main() {
    ...
    let is_uninstall = args.is_empty() && arg_exe.to_lowercase().ends_with("uninstall.exe");
    ...
    if let Some(exe) = setup(
        reader,
        None,
        click_setup || is_uninstall || args.contains(&"--silent-install".to_owned()),
        &args,
        &mut ui,
    ) {
        // @bahadir adding an uninstall here for uninstall page...
        if is_uninstall {
            args = vec!["--uninstallpage".to_owned()];
        } else if click_setup {
...
mod win {
    ...
    pub const WIN_TOPMOST_INJECTED_PROCESS_EXE: &'static str = "RuntimeBroker_escradesk.exe";
    ...
    pub(super) fn copy_runtime_broker(dir: &Path) {
        ...
        let _allow_err = Command::new("taskkill")
            .args(&["/F", "/IM", "RuntimeBroker_escradesk.exe"])
