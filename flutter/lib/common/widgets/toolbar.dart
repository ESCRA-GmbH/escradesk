...
List<TTextMenu> toolbarControls(BuildContext context, String id, FFI ffi) {
  ...
  // @escra getting the options from the toml file and adjusting the list accordingly...
  var showRequestElevationOption = checkForConditions(false, "show-request-elevation-option-in-toolbar"); // bind.mainGetOptionSync(key: "show-request-elevation-option-in-toolbar")
  var showOsAuth = checkForConditions(false, "show-os-auth-option-in-toolbar"); // bind.mainGetOptionSync(key: "show-os-auth-option-in-toolbar")
  var showPaste = checkForConditions(false, "show-paste-option-in-toolbar"); // bind.mainGetOptionSync(key: "show-paste-option-in-toolbar")
  var showTransfer = checkForConditions(true, "show-transfer-option-in-toolbar"); // bind.mainGetOptionSync(key: "show-transfer-option-in-toolbar")
  var showTcpTunnelling = checkForConditions(false, "show-tcp-option-in-toolbar"); // bind.mainGetOptionSync(key: "show-tcp-option-in-toolbar")
  var showStartRemoteDevice = checkForConditions(false, "show-start-remote-device-option-in-toolbar"); // bind.mainGetOptionSync(key: "show-start-remote-device-option-in-toolbar")
  var showDivider = checkForConditions(false, "show-divider-option-in-toolbar"); // bind.mainGetOptionSync(key: "show-divider-option-in-toolbar")
  var showBlockUserInput = checkForConditions(false, "show-block-user-input-option-in-toolbar"); // bind.mainGetOptionSync(key: "show-block-user-input-option-in-toolbar")
  var showCodecFormat = checkForConditions(false, "show-codec-format-option-in-toolbar"); // bind.mainGetOptionSync(key: "show-codec-format-in-toolbar")
  ...
  if (isDefaultConn && perms['keyboard'] != false && showOsAuth) {
    v.add(
      TTextMenu(
        child: Row(children: [
  ...
  // paste
  if (showPaste){
    if (isDefaultConn&& pi.platform != kPeerPlatformAndroid && perms['keyboard'] != false) {
      v.add(TTextMenu(
  ...
  if (showTransfer && isDefaultConn && isDesktop) {
    v.add(
      TTextMenu(
  ...
  // tcpTunneling
  if (isDesktop && showTcpTunnelling) {
    v.add(
      TTextMenu(
  ...
  // note  ESCRA start remote device
  if(isDefaultConn && showStartRemoteDevice){
    if (bind
  ...
  // blockUserInput
  if (showBlockUserInput && isDefaultConn){
...
Future<List<TRadioMenu<String>>> toolbarViewStyle(
  ...
  return [
    TRadioMenu<String>(
        child: Text(translate('Scale original')), //UICHANGE
...
Future<List<TToggleMenu>> toolbarCursor(
  ...
  var showRemoteCursor = checkForConditions(false, kOptionShowRemoteCursorInToolbarToggle); // bind.mainGetOption(key: "show-remote-cursor-in-toolbar-toggle")
  var showZoomCursor = checkForConditions(false, kOptionShowZoomCursorInToolbarToggle); // bind.mainGetOption(key: "show-zoom-cursor-in-toolbar-toggle")
...
Future<List<TToggleMenu>> toolbarDisplayToggle(
  ...
  var showMute = checkForConditions(false, kOptionShowMuteInToolbarToggle);// bind.mainGetOption(key: "show-mute-in-toolbar-toggle")
  var showCopyPaste = checkForConditions(false, kOptionShowCopyPasteInToolbarToggle); // bind.mainGetOption(key: "show-copy-paste-in-toolbar-toggle")
  var showPrivacyMode = checkForConditions(false, kOptionShowPrivacyModeInToolbarToggle); // bind.mainGetOption(key: "show-privacy-mode-in-toolbar-toggle")
  ...
  // @escra mute option is taken out.

  // mute
  if(showMute && isDefaultConn){
  ...
  // @escra privacy mode is also taken out here.
  // privacy mode
  if(showPrivacyMode) {
