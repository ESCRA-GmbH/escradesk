...
class _PeerTabPageState extends State<PeerTabPage>
  ...
  // @escra
  // creating the view properties to configure the views later on.
  var optionRecentPeersView = false;
  var optionFavoritePeersView = false;
  var optionDiscoverView = false;
  var optionAddressBookView = false;
  var optionMyGroupView = false;
  var optionAllViews = true;
  var optionDefaultViews = false;
  ...
  final List<_TabEntry> entries = [
    ...
    _TabEntry(DiscoveredPeersView(
      menuPadding: _menuPadding(),
        ...
        // @escra removing the addressbook
    _TabEntry(
       AddressBook(
         menuPadding: _menuPadding(),
       ),
  ...
  void _loadLocalOptions() {
    ...
    // @escra
    // here we get the options from the file into the state to config the views accordingly
    // todo still you need to adapt the configs that you get. Its not connected to the program front end yet.
    gettingOptionsFromConfigFile() async {
    Map<String, dynamic> options =
          jsonDecode(bind.mainGetOptionsSync());
      optionRecentPeersView = options["optionRecentPeersView"] == "true" ? true : optionRecentPeersView;
      optionFavoritePeersView = options["optionFavoritePeersView"] == "true" ? true: optionFavoritePeersView;
      optionDiscoverView = options["optionDiscoverView"] == "true" ? true : optionDiscoverView;
      optionAddressBookView = options["optionAddressBookView"] == "true" ? true : optionAddressBookView;
      optionMyGroupView = options["optionMyGroupView"] == "true" ? true : optionMyGroupView ;
      optionAllViews = options["optionAllViews"] == "true" ? true : optionAllViews;
      optionDefaultViews = options["optionDefaultViews"] == "true" ? true : optionDefaultViews;
    }
