...
//   // @escra when we dont want to add api-server in the servers.dart then we might need the following...

//   // @escra even though its not able to be connected we want the change anyways...
...
//   await bind.mainSetOption(
//       key: 'custom-rendezvous-server', value: config.idServer);
//   // @escra added the following line not to confuse the servers.
//   //await bind.mainSetOption(
//   //    key: 'rendezvous_server', value: config.idServer);
//   await bind.mainSetOption(key: 'relay-server', value: config.relayServer);
//   await bind.mainSetOption(key: 'api-server', value: config.apiServer);
//   await bind.mainSetOption(key: 'key', value: config.key);

//   debugPrint(" custom server now: ${await bind.mainGetOption(key: 'custom-rendezvous-server')}");
...
List<Widget> ServerConfigImportExportWidgets(
  ...
  return [
    Tooltip(
      ...
      child: IconButton(
          icon: Icon(Icons.paste, color: Colors.grey), onPressed: import), // HARDCODEDCOLOR
    ),
    Tooltip(
        message: translate('Export Server Config'),
        child: IconButton(
            icon: Icon(Icons.copy, color: Colors.grey), onPressed: export)) // HARDCODEDCOLOR
  ];
}
