...
class PlatformFFI {
  ...
  EscradeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libescradesk.so')
        : isLinux
            ? DynamicLibrary.open('libescradesk.so')
            : isWindows
                ? DynamicLibrary.open('libescradesk.dll')
                :
  ...
  void _startListenEvent(EscradeskImpl escradeskImpl) {
    final appType = _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = escradeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
