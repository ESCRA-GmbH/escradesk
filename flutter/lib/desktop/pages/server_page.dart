    ...
    return Container(
      decoration: BoxDecoration(
        ...
        gradient: LinearGradient(
          ...
          colors: [
            //Color(0xff00bfe1), //WILL CHANGE HERE
            //Color(0xff0071ff), // CHECK LATER ON
            MyTheme.accent,
            MyTheme.button // CHANGED HERE: you can see the old colors
            //Color(0xff00bfe1), //WILL CHANGE HERE
            //Color(0xff0071ff), // CHECK LATER ON
          ],
        ),
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              color: str2color(client.name), // HARDCODEDCOLOR // SHOULD LOOK HERE
...
class _PrivilegeBoardState extends State<_PrivilegeBoard> {
  ...
  // @escra if we want to make them configurable from the toml file,
  // we can easily get the value from the toml file and then apply.

  // when the value in the toml file is different than the 'N' it becomes true.
  var showKeyboardOption = checkForConditions(true, "show-keyboard-option-in-control-panel"); // bind.mainGetOptionSync(key: "show-keyboard-option-in-control-panel"); //
  var showClipboardOption = checkForConditions(true, "show-clipboard-option-in-control-panel"); // bind.mainGetOptionSync(key: "show-clipboard-option-in-control-panel");
  var showAudioOption = checkForConditions(true, "show-audio-option-in-control-panel"); // bind.mainGetOptionSync(key: "show-audio-option-in-control-panel");
  var showFileCopyPasteOption = checkForConditions(true, "show-copy-paste-option-in-control-panel"); // bind.mainGetOptionSync(key: "show-file-copy-paste-option-in-control-panel");
  var showRemoteStartOption = checkForConditions(true, "show-remote-start-option-in-control-panel"); // bind.mainGetOptionSync(key: "show-remote-start-option-in-control-panel");
  var showRecordingOption = checkForConditions(true, "show-recording-option-in-control-panel"); // bind.mainGetOptionSync(key: "show-recording-option-in-control-panel");
...
class _CmControlPanel extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        ...
        Offstage(
          ...
          child: buildButton(context,
              color: Colors.purple, // HARDCODEDCOLOR
              onClick: () => handleSwitchBack(context),
              icon: Icon(Icons.reply, color: Colors.white),
              text: "Switch Sides",
              textColor: Colors.white),
        ),
        /* UICHANGES disable evaluate button after connected
        Offstage(
  ...
  buildUnAuthorized(BuildContext context) {
    ...
    return Column(
      mainAxisAlignment: MainAxisAlignment.end,
      children: [
        // @escra the accept and elevate button is commented out.
        // Offstage(
  ...
  void handleVoiceCall(bool accept) {
    ...
    // @escra here we will add, recording option...
    if (accept) {
