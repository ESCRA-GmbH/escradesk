...
// @escra I am planning to add the different server options from the file.
import 'package:flutter_hbb/servers.dart';
...
const String keyInternalServerUsage = 'internal-server-usage';
...
// if you want to change the tabs, you can change it here.
const defaultTabsList = [SettingsTabKey.general, SettingsTabKey.safety, SettingsTabKey.network, SettingsTabKey.display, /*SettingsTabKey.printer,*/ SettingsTabKey.about];

// Return true if the key is true.
bool getOption(String key) {
  return bind.mainGetOptionSync(key: key) == 'Y';
}
...
class DesktopSettingPage extends StatefulWidget {
  ...
  static final List<SettingsTabKey> tabKeys = [
    if (!getOption(kOptionHideGeneralSettingTab))
    SettingsTabKey.general,
    if (!isWeb &&
        !bind.isOutgoingOnly() &&
        !bind.isDisableSettings() &&
        bind.mainGetBuildinOption(key: kOptionHideSecuritySetting) != 'Y' &&
        !getOption(kOptionHideSafetySettingTab))
      SettingsTabKey.safety,
    if (!bind.isDisableSettings() &&
        bind.mainGetBuildinOption(key: kOptionHideNetworkSetting) != 'Y' &&
        !getOption(kOptionHideNetworkSettingTab))
      SettingsTabKey.network,
    if (!bind.isIncomingOnly() && !getOption(kOptionHideDisplaySettingTab) ) SettingsTabKey.display,
    if ((!isWeb && !bind.isIncomingOnly() && bind.pluginFeatureIsEnabled()) || getOption(kOptionShowPluginSettingTab) )
      SettingsTabKey.plugin,
    if ((!bind.isDisableAccount() && bind.mainGetBuildinOption(key: kOptionHideDisplaySetting) != 'Y') || getOption(kOptionShowAccountSettingTab)) SettingsTabKey.account,
    if (!bind.isDisableAccount()) SettingsTabKey.account,
    if (isWindows &&
        bind.mainGetBuildinOption(key: kOptionHideRemotePrinterSetting) != 'Y')
      SettingsTabKey.printer,
    if (!getOption(kOptionHideAboutSettingTab)) SettingsTabKey.about,
  ];
...
class _DesktopSettingPageState extends State<DesktopSettingPage>
  ...
  // @escra
  // Here are the configs for the settings page
  dynamic defaultTabs ;
  dynamic allTabs ;
  dynamic extraOptionsInApp; // for all options
  dynamic optionSecurity; // for security tab
  dynamic optionNetwork; // for network tab
  dynamic optionDisplay; // for display tab
  dynamic optionPlugin; // for plugin tab
  dynamic optionAccount; // for account tab
  dynamic optionAbout; // for about tab
  ...
  _DesktopSettingPageState(SettingsTabKey initialTabkey) {
    ...
    gettingOptionsFromConfigFile() async {
      Map<String, dynamic> options =
          jsonDecode(bind.mainGetOptionsSync());
      optionSecurity = options["optionSecurity"] == "true";  // ? true : optionSecurity;
      optionNetwork = options["optionNetwork"] == "true";  // ? true : optionNetwork;
      optionDisplay = options["optionDisplay"] == "true";  // ? true : optionDisplay;
      optionPlugin = options["optionPlugin"] == "true";  // ? true : optionPlugin;
      optionAccount = options["optionAccount"] == "true";  // ? true : optionAccount;
      optionAbout = options["optionAbout"] == "true";  // ? true : optionAbout;
      allTabs = options["allTabs"] == "true";  // ? true : allTabs;
      defaultTabs = options["defaultTabs"] == "true";  // ? true : defaultTabs;
      // debugPrint("InitState Calls: ");
      // debugPrint(options.toString());

      // for development purposes
      //debugPrint("Please notice that allTabs are true by the source code now!");
      defaultTabs = true;
      // allTabs = true;
    }

    // settingDefaultOptions();
    gettingOptionsFromConfigFile();
  }
  ...
  List<_TabInfo> _settingTabs() {
    ...
    for (final tab in DesktopSettingPage.tabKeys) {
      if (!defaultTabsList.contains(tab)) continue;
      switch (tab) {
...
class _SafetyState extends State<_Safety> with AutomaticKeepAliveClientMixin {
  ...
  Widget permissions(context) {
    ...
    tmpWrapper() {
      ...
      return _Card(title: 'Permissions', children: [ // LOGIC MERGE CHANGE
        ComboBox(
            keys: [
...
class _Servers extends StatefulWidget {
   const _Servers({Key? key}) : super(key: key);

  @override
  State<_Servers> createState() => _ServersState();
}

class _ServersState extends State<_Servers> with AutomaticKeepAliveClientMixin {
  @override
  bool get wantKeepAlive => true;
  bool locked = bind.mainIsInstalled();
  // SHOULD LOOK HERE - how do we want to start to the app?
  ServerType serverType = bind.mainGetOptionSync(key: "connection-type") == "internal" ? ServerType.internal : ServerType.external;
  Map<String, ServerConfig> servers = Servers.serversList();
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
        child: ListView(
            ...
            physics: NeverScrollableScrollPhysics(),
            children: [
              _lock(locked, 'Unlock Servers Settings', () {
                locked = false;
                setState(() => {});
              }),
              AbsorbPointer(
                ...
                child: Column(children: [
                  ...
                  const SizedBox(width: 24, height: 24),
                    DropdownMenu<String>(
                  initialSelection: bind.mainGetOptionSync(key:"serverType"), // @escra here should be checked what happens if no option is there.
                  ...
                  onSelected: (String? server) {
                    setState(() {
                      changeTheServer(server!);
                    });
                  },
                  dropdownMenuEntries: servers.keys
                      .map<DropdownMenuEntry<String>>(
                          (String serverName) {
                        return DropdownMenuEntry<String>(
                          value: serverName,
                          label: serverName,
                          // @escra we can also enable some options here...
                          // enabled: color.label != 'Grey',
                          style: MenuItemButton.styleFrom(
                          backgroundColor: Theme.of(context).scaffoldBackgroundColor,
                          // @escra we can also change the text color.
                          // foregroundColor:
                          ),
                        );
                      }).toList(),
  ...
  void changeTheServer(String server) async{
    ...
    List <TextEditingController> placeholder = [];

    if ( await setServerConfig(placeholder, errMsgs, servers[server]!) ) {
      bind.mainSetOption(key: "connection-type", value: server);
      showToast("The server is changed to: $server ");
    }
    else {
      bind.mainSetOption(key: "connection-type", value: server);
      showToast("The server $server is not able to be connected.");
    }
    // if ( await setServerConfig(placeholder, errMsgs, servers[server]!) ) {
    //   bind.mainSetOption(key: "connection-type", value: server);
    //   if (server == 'internal') {
    //     bind.mainSetOption(key: "internal-server-usage", value: "true");
    //   }
    //   else {
    //     bind.mainSetOption(key: "internal-server-usage", value: "false");
    //   }
    // }
  }
...
class _DisplayState extends State<_Display> {
  var showCodec = checkForConditions(false, "show-codec");
  // var showCodec = option2bool("show-codec", bind.mainGetOptionSync(key: "show-codec") ) ;
...
// POSSIBLE BRANDING HERE
class _AboutState extends State<_About> { // WILL CHANGE HERE -- should change the link and the descriptions here in the setting page soon
  @override
  Widget build(BuildContext context) {
    return futureBuilder(future: () async {
      ...
      return SingleChildScrollView(
        controller: scrollController,
        child: _Card(title: translate('About EscraDesk'), children: [ // CHANGED HERE BRANDING - license is also not used here!
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const SizedBox(
              ...
              // if (!isWeb)
                //SelectionArea(
                    //child: Text('${translate('Fingerprint')}: $fingerprint')
                        //.marginSymmetric(vertical: 4.0)),
                  // MERGE CHANGE DELETED FINGERPRINT
              InkWell(
                  onTap: () {
                    launchUrlString('https://www.escra.de/privacy-policy');
                  },
                  child: Text(
              ...
              InkWell(
                  onTap: () {
                    launchUrlString('https://www.escra.de');
                  },
                  child: Text(
                    translate('Website'),
                    style: linkStyle,
              ...
              Container(
                decoration: const BoxDecoration(color: MyTheme.accent), // HARDCODEDCOLOR // CHANGED HERE: It was blue before now I made theme color
                padding:
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            'Copyright © 2025 Purslane Ltd.\nCopyright © 2025 Escra Ltd.\n',
                            style: const TextStyle(color: Colors.white),
                          ),
                          Text(
                            translate('Slogan_tip'), // POSSIBLE BRANDING
                            style: TextStyle(
                                fontWeight: FontWeight.w800,
                                color: Colors.white), // HARDCODEDCOLOR
                          )
                        ],
                      ),
                    ),
