...
const char* uniLinksPrefix = "escradesk://";
typedef int (*FUNC_RUSTDESK_GET_APP_NAME)(wchar_t*, int);
/// Note: `--server`, `--service` are already handled in [core_main.rs].
const std::vector<std::string> parameters_white_list = {"--install", "--uninstallpage", "--cm"};
...
{
  HINSTANCE hInstance = LoadLibraryA("libescradesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libescradesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
