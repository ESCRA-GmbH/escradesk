...
oLink.Save
    ...
    let uninstall_with_data_shortcut = write_cmds(
        format!(
            "
Set oWS = WScript.CreateObject(\"WScript.Shell\")
sLinkFile = \"{tmp_path}\\Uninstall With Data {app_name}.lnk\"
Set oLink = oWS.CreateShortcut(sLinkFile)
    oLink.TargetPath = \"{exe}\"
    oLink.Arguments = \"--uninstall-with-data\"
    oLink.IconLocation = \"msiexec.exe\"
oLink.Save
        "
        ),
        "vbs",
        "uninstall_with_data_shortcut",
    )?
    .to_str()
    .unwrap_or("")
    .to_owned();
    ...
    // @escra if we want to make it default maybe we can add it to the start menu, we can talk about it
    // by default, it does not delete the user data.
    if options.contains("startmenu") {
        shortcuts = format!(
            "{shortcuts}
md \"{start_menu}\"
copy /Y \"{tmp_path}\\{app_name}.lnk\" \"{start_menu}\\\"
copy /Y \"{tmp_path}\\Uninstall With Data {app_name}.lnk\" \"{start_menu}\\\"
copy /Y \"{tmp_path}\\Uninstall {app_name}.lnk\" \"{start_menu}\\\"
    ...
    // @escra change for the remove user data
    let mut remove_user_data = Default::default();
    if options.contains("removeuserdata"){
        remove_user_data = format!(" rd /s /q \"%APPDATA%\\{app_name}\\\"
        rd /s /q \"C:\\Windows\\ServiceProfiles\\LocalService\\AppData\\Roaming\\{app_name}\\\"
        rd /s /q \"{tmp_path}\\{app_name}\\\"
        sc stop {app_name}
        sc delete {app_name}
        ...
        filter = format!(" /FI \"PID ne {}\"", get_current_pid())
        );
    }
...
if exist \"{uninstall_with_data_shortcut}\" del /f /q \"{uninstall_with_data_shortcut}\"
if exist \"{tray_shortcut}\" del /f /q \"{tray_shortcut}\"
if exist \"{tmp_path}\\{app_name}.lnk\" del /f /q \"{tmp_path}\\{app_name}.lnk\"
if exist \"{tmp_path}\\Uninstall With Data {app_name}.lnk\" del /f /q \"{tmp_path}\\Uninstall With Data {app_name}.lnk\"
...
cscript \"{uninstall_with_data_shortcut}\"
{tray_shortcuts}
{shortcuts}
copy /Y \"{tmp_path}\\Uninstall With Data {app_name}.lnk\" \"{path}\\\"
...
{sleep}
    ",
        ...
        // @escra adding remove user data property in the string
    ...
    // // trying to do the import config after install.
    // let mut cur_dir = std::env::current_dir().unwrap();
    // let custom_toml = cur_dir.join("EscraDesk.toml").to_str().unwrap_or("").to_owned();
    // crate::core_main::import_config_pub(&custom_toml);
...
fn get_uninstall_with_data(kill_self: bool) -> String {

    let remove_user_data = format!(" rd /s /q \"%APPDATA%\\{app_name}\\\"
        rd /s /q \"C:\\Windows\\ServiceProfiles\\LocalService\\AppData\\Roaming\\{app_name}\\\"
        rd /s /q \"{tmp_path}\\{app_name}\\\"
        ",
        app_name = crate::get_app_name(),
        tmp_path = std::env::temp_dir().to_string_lossy().to_string(),
        );
...
pub fn uninstall_me_with_data(kill_self: bool) -> ResultType<()> {
    run_cmds(get_uninstall_with_data(kill_self), true, "uninstall_with_data")
}
...
pub fn get_license_from_exe_name() -> ResultType<CustomServer> {
    ...
    // @escra the name of the file might be changing here, lets try without changing the file
...
pub fn bootstrap() -> bool {
        // @escra , set the rendezvous server from exe name
...
if exist \"{tray_shortcut}\" del /f /q \"{tray_shortcut}\"
    ...
    log::debug!("Install service running...");
...
pub fn install_service_with_path(path: &str) -> bool {
...
if exist \"{tray_shortcut}\" del /f /q \"{tray_shortcut}\"
    ",
        app_name = crate::get_app_name(),
        import_config = get_import_config_with_path_change_service(&exe, &path),
        create_service = get_create_service(&exe),
    );
    ...
    // Config::set_option("alo-alo".to_string(), "y".to_string());
    // let config_changed = Config::get_option("config-changed");
    // if config_changed.is_empty() {
    //     crate::license::set_default_license();
    //     log::info!("Set default license");
    // }
...
fn get_import_config(exe: &str) -> String {
    ...
    // @escra adding the windows.rs import changes from old code

    let mut cur_dir = std::env::current_dir().unwrap();
    let cur_dir_str = cur_dir.to_str().expect("Get import config error");

    let custom_toml = cur_dir.join("EscraDesk.toml").to_str().unwrap_or("").to_owned();
    log::info!("custom.toml path: {:?}", custom_toml);
    format!("
sc stop {app_name}
sc delete {app_name}
sc create {app_name} binpath= \"\\\"{exe}\\\" --import-config-service \\\"{custom_toml}\\\"\" start= auto DisplayName= \"{app_name} Service\"
sc start {app_name}
sc stop {app_name}
sc delete {app_name}
",
...
/**
 * This function is creating service and makes the changes there.
 * It takes an exe and path, which will be used for import-config service.
 * Exe is to execute and path is file itself with escradesk.toml .
 */
pub fn get_import_config_with_path_change_service(exe: &str, path: &str) -> String {
    // @escra adding the windows.rs import changes from old code

    format!("
sc stop {app_name}
sc delete {app_name}
sc create {app_name} binpath= \"\\\"{exe}\\\" --import-config-service \\\"{path}\\\"\" start= auto DisplayName= \"{app_name} Service\"
sc start {app_name}
sc stop {app_name}
sc delete {app_name}
",
...
fn get_license() -> Option<CustomServer> {
    ...
    log::info!("Hoca noliy");
    log::info!("{}", format!("{:#?}", (&lic.key, &lic.host, &lic.api)));
