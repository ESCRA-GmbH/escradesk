...
use crate::{
    ...
    }, common::send_videos_to_server, display_service, ipc, privacy_mode, video_service, VERSION
};
...
impl Connection {
    pub async fn start(
        ...
        let mut conn = Self {
            ...
            // @escra enable_file_transfer and show_remote_cursor by default...
            show_remote_cursor: true,
            follow_remote_cursor: false,
            follow_remote_window: false,
            multi_ui_session: false,
            ip: "".to_owned(),
            disable_audio: true, // UICHANGES
            #[cfg(any(target_os = "windows", target_os = "linux", target_os = "macos"))]
            enable_file_transfer: false, //UICHANGES
    ...
    // @escra , checking whether the client is connectable
    async fn check_connectable(&mut self, addr: &SocketAddr) -> bool {
        let connectable = Config::get_option("connectable") != "N" ; 
        if !connectable {
            self.send_login_error(format!("The peer is not connectable. Id: {}", Config::get_id()))
                .await;
            Self::post_alarm_audit(
                AlarmAuditType::NotConnectable, 
                // @escra , I dont know why we send the ip here? 
                json!({ "id": self.inner.id })); 
            return false; 
        }
        true
    }
    ...
    async fn on_open(&mut self, addr: SocketAddr) -> bool {
        ...
        // @escra , adding being connectable to the machine 
        if !self.check_connectable(&addr).await {
            return false; 
        }
