...
fn get_recorder(
    ...
    let recorder = if record_incoming {
        ...
        // Check if the option has changed to do uploading record 
        // then change the record_upload.ENABLE
        // changing the record_upload
        let auto_upload = !Config::get_option("allow-auto-upload-record").is_empty(); 
        if record_upload::is_enable() != auto_upload {
            record_upload::change_enable(auto_upload);
        }
