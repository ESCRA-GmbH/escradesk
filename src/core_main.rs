...
// @escra macro to print arguments
#[macro_export]
...
pub fn core_main() -> Option<Vec<String>> {
    ...
    use crate::{custom_server::set_default_license, platform::{install_service}};
    #[cfg(target_os = "windows")] 
    use crate::platform::install_service_with_path;
    ...
    // @escra
    // @escra, before starting, make sure the arguments are there.

    set_default_license();


    log::info!("core_main has been called with args {:#?}", args);
    ...
    // @escra  adding an option in the uninstall
    let click_setup = cfg!(windows) && args.is_empty() && crate::common::is_setup(&arg_exe);
    if click_setup && !config::is_disable_installation() {
        if crate::common::is_uninstall(&arg_exe) {
            log::info!("Uninstall argument is added");
            args.push("--uninstallpage".to_owned());
            flutter_args.push("--uninstallpage".to_string());
        } else {
            log::info!("Install argument is added.");
    ...
    if args.is_empty() || crate::common::is_empty_uni_link(&args[0]) {
        ...
        std::thread::spawn(move || crate::send_videos_to_server()); 
    } else {
        #[cfg(windows)]
        {
            use crate::platform;
            if args[0] == "--uninstall" {
                ...
                return None;
            } else if args[0] == "--uninstall-with-data" {
                if let Err(err) = platform::uninstall_me_with_data(true) {
                    log::error!("Failed to uninstall: {}", err);
                }
                return None;
        ...
        if args[0] == "--remove" {
            ...
            if args.len() == 2 {
                ...
                #[cfg(windows)]
                if crate::platform::is_installed() {
                    install_service_with_path(&filepath);
                    return None;
                    // crate::platform::get_import_config_with_path_change_service(crate::platform::, &filepath)
                }
        ...
        else if args[0] == "--import-config-service" {
            if args.len() == 2 {
  ...
  --device_name"#
    ...
    #[cfg(feature = "flutter")]
    log::info!("Using flutter arguments:");
    log::info!("{:?}", flutter_args);
    return Some(flutter_args);
    #[cfg(not(feature = "flutter"))]
    log::info!("Using normal arguments:");
    log::info!("{:?}", args);
    return Some(args);
...
/*
 * @escra , before the program begins, we first set the default config
 * if there is any import_config, it would not be affected since this function
 * would be run before
*/
pub fn set_default_config() {

    crate::custom_server::set_default_license();

    let config_changed = Config::get_option("config-changed");
    if config_changed.is_empty() {
        crate::common::set_escra_options();
        log::info!("Set default config");


    } else {
        log::info!("Using old config");
    }
    #[cfg(windows)]{
...
fn import_config(path: &str) {
    ...
    if store_path(Config::file(), config).is_err() {
        ...
        // Config::set_option("path".to_string(), path.to_string_lossy().to_string());
    }

    let config2: Config2 = load_path(path2.into());
    // if we want to hold only the things in target toml
    // Config2::default();
    Config::set_options(config2.options);



    // @escra we are changing the conditions to add the import.
    // we dont check the time now.
    ...
    // let config2: Config2 = load_path(path2.into());
    // // Config::set_options(config2.options);
    // Config::set_option("config2".to_string(), format!("{:?}", &config2));
    // Config::set_option("config2filepath".to_string(), Config2::file().to_string_lossy().to_string());
    ...
    //     } else {
    //         let config22: Config2 = load_path(path2.into());
    //         store_path(Config2::file(), config22);
    //         Config::set_option("path2".to_string(), path2.to_string_lossy().to_string());
    //     }
    // //}

    Config::set_option("config-changed".to_string(), "Y".to_string());
    set_default_config();
}

// @escra creating the same function with pub
pub fn import_config_pub(path: &str) {
    use hbb_common::{config::*, get_exe_time, get_modified_time};
    let path2 = path.replace(".toml", "2.toml");
    let path2 = std::path::Path::new(&path2);
    let path = std::path::Path::new(path);
    ...
    // }
    Config::set_option("core-main-config-pub-changed".to_string(), "Y".to_string());
}
