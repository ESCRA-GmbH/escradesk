...
use crate::{
    hbbs_http::{create_http_client_async, get_url_for_tls}, platform::is_root, ui_interface::{get_option, set_option, video_save_directory}
};
...
pub const PORTABLE_APPNAME_RUNTIME_ENV_KEY: &str = "ESCRADESK_APPNAME";
...
pub fn is_uninstall(name: &str) -> bool {
    name.to_lowercase().ends_with("uninstall.exe")
}
...
// The default options that we want to set for the app
pub fn set_escra_options() {
    Config::set_option("enable-keyboard".to_string(), "N".to_string());
    Config::set_option("enable-clipboard".to_string(), "N".to_string());
    Config::set_option("allow-remote-config-modification".to_string(), "Y".to_string());
    Config::set_option("view_style".to_string(), "adaptive".to_string());
    Config::set_option("theme".to_string(), "dark".to_string());
    // Config::set_option("force-always-relay".to_string(), "Y".to_string());
...
// Check if all the videos on the directory has been send to the server
// if not send them 
pub fn send_videos_to_server() {
        // check directory
        // get all the videos which is not send. 
        let videos_to_send = videos_to_send(); 
        match videos_to_send {
            None => log::info!("There is no video to send"),
            Some(videos) => {
                for video in videos {
                    match send_video_to_server(&video) {
                        Ok(_) => log::info!("{} is send to the server", &video),
                        Err(err) => log::error!("{} is not send to the server because of {}", video, err),
                    }
                }
            }   
        }
        ...
        // for each video, send it to the server. 
        // if there is a error in the server or somewhere then return 
}

fn videos_to_send() -> Option<Vec<String>>  {

    let path = video_save_directory(is_root()); // Get the correct path 
    let mut dir = std::fs::read_dir(path).unwrap(); 

    let mut result: Vec<String> = Vec::new();
    log::info!("Checking the directory for the videos to send");
    ...
    while let Some(entry) = dir.next() {
        ...
        log::info!("{} being checked", file_name);

        // the condition for the file to be send
        if (file_name.starts_with("incoming") || file_name.starts_with("outgoing")) {
            result.push(path.to_str().unwrap().to_string());
            println!("{} is not send to the server before", file_name); 
            continue;
        }
        log::info!("{} is not to be send", file_name);
    }
...
fn send_video_to_server(video: &String) -> ResultType<()> {

    log::info!("{} is being checked", &video);
    let path = std::path::Path::new(&video);
    let file_name = path.file_name().unwrap().to_string_lossy().to_string();
    let file = std::fs::read(&video); 
    match file {
        Ok(_) => log::info!("{} is read", &video),
        Err(err) => {
            log::error!("{} is not read because of {}", &video, err);
            return Err(anyhow!("Could not read the file"));
        }
    }

    let part = reqwest::blocking::multipart::Part::bytes(file?).file_name(file_name.clone());
    // Create a multipart form to send both the file and the file name
    let form = reqwest::blocking::multipart::Form::new()
        .text("file_name", file_name.to_string())  // Send the file name as a form field
        .part("file", part);  // Attach the file

    // Send the request to the server
    let client = reqwest::blocking::Client::builder().timeout(Duration::from_secs(300)).tcp_keepalive(Some((Duration::from_secs(10)))) // 5-minute timeout
    .build(); 

    let client = match client {
        Ok(client) => client,
        Err(e) => {
            log::error!("Client is not built {}", e);
            return Err(anyhow!("Could not build the client"));
        }
    };

    let response = client
        .post("http://0.0.0.0/upload")
        .multipart(form)
        .send();

    // Changing the video info here.
    match response {
        Ok(resp) => { 
            let response = resp.text().unwrap();
            log::info!("{:?}", response);

            if response.starts_with("File uploaded successfully") {
                log::info!("{} is uploaded successfully", &video);
                let new_path = path.with_file_name(format!("s_{}", file_name));
                match std::fs::rename(path, &new_path){
                    Ok(_) => log::info!("{} is renamed", &path.display()),
                    Err(k) => log::error!("Could not change the name of the file. {k} , the file is {}. New file is {}", &path.display(), &new_path.display())
                }
                return Ok(());
            } else {
                log::error!("{} is not uploaded successfully", &video);
            }
            // Change the video info here.
            
    } ,
        Err(err) => log::error!("Failed to get response text: {:?}", err),
    }

    return Err(anyhow!("Could not send the video to the server"));
}
...
pub fn is_custom_client() -> bool {
    // todo check out if you can simple can change names here
    get_app_name() != "EscraDesk"
}
